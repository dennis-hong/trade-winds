...
export function WorldMap({ currentCity, gold, crew, onTravel }: WorldMapProps) {
  ...
  return (
    <div className="ocean-card rounded-xl p-3 md:p-4">
      ...
      {/* 범례 */}
      <div className="flex flex-wrap gap-2 md:gap-3 mb-2 md:mb-3 text-[10px] md:text-xs">
        ...
        >
          ...
          {Object.entries(CITY_POSITIONS).map(([city, pos]) => {
            ...
            return (
              ...
              >
                {/* 투명 터치 타겟 (모바일용 - 더 넓은 클릭 영역) */}
                <circle
                  r="24"
                  fill="transparent"
                  style={{ pointerEvents: connected || isCurrent ? 'auto' : 'none' }}
                />
      ...
      {/* 현재 위치 정보 */}
      <div className="mt-2 md:mt-3 p-2.5 md:p-3 rounded-lg bg-[#0c1929]/50 border border-[#2d5a87]">
        <div className="flex items-center gap-2 text-[#c9a227]">
          <MapPin className="w-4 h-4 md:w-5 md:h-5" />
          <span className="font-bold text-sm md:text-base">현재: {currentCity}</span>
          <span className="text-[10px] md:text-xs text-[#8b6914] ml-auto">
            → {connectedCities.join(', ')}
          </span>
        </div>
      </div>
    </div>
